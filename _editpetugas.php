...
if (isset($_POST['editpetugas'])) {
    ...
    if (!empty($_FILES['pic']['name'])) {
        ...
        $dir = "pengurus/";
        $foto = $dir . $namafoto;
        if(!empty($password)) $query = "UPDATE pengurus SET nama = '$nama', password = '$password', level = '$lvl', pic = '$foto' WHERE id_pengurus='$id'";
        else $query = "UPDATE pengurus SET nama = '$nama', level = '$lvl', pic = '$foto' WHERE id_pengurus='$id'";
        ...
        if(!empty($password)) $query = "UPDATE pengurus SET nama = '$nama', password = '$password', level = '$lvl' WHERE id_pengurus='$id'";
        else $query = "UPDATE pengurus SET nama = '$nama', level = '$lvl' WHERE id_pengurus='$id'";
        $sql = mysqli_query($koneksi, $query);
        if ($sql) {
            echo "<script>
                swal('Data Berhasil Diedit!', '', 'success').then(function(){
            ...
            <form action="dashboard.php?page=editpetugas" method="post" name="editpetugas" enctype="multipart/form-data">
                <div class="row">
                    ...
                    <div class="col-sm-9 text-secondary">
                        <input type="text" name="nama" id="nama" class="form-control" placeholder="Masukan Nama" value="<?php echo $data['nama'] ?>" required>
                    </div>
                </div>
                <hr>
                <div class="row">
                    ...
                    <div class="col-sm-9 text-secondary">
                        <input type="radio" name="lvl" id="owner" value="owner" <?php if($data['level']=="owner") echo "checked" ?> required>
                        <label for="owner">Owner</label><br>
                        <input type="radio" name="lvl" id="petugas" value="petugas" <?php if($data['level']=="petugas") echo "checked" ?> required>
